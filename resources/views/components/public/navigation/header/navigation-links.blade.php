@php
    ...
    use App\Enums\AvailableLanguage;$items = [
...
<ul class="flex flex-col gap-6 md:flex-row md:items-center md:justify-end">
    @foreach($items as $item)
        <li class="flex">
            <x-public.navigation.header.navigation-link :destination="$item['destination']" :title="$item['title']"
                                                        :class="$item['class']">
                {!! $item['label'] !!}
    ...
    <li class="max-md:mt-auto ">
        @foreach(AvailableLanguage::cases() as $locale)
            @if($locale->value !== app()->getLocale())
                <a title="Changer la langue en {{ __('enum.' . $locale->value) }}"
                   class="text-2xl md:text-base font-normal md:font-medium hover:font-bold active:font-bold trans-all"
                   href="{{ route('public.locale', $locale->value) }}">{{ __('enum.' . $locale->value) }}</a>
            @endif
        @endforeach
    </li>
</ul>
